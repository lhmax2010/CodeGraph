...
- 2026-07-08 P7 review MAJOR 修复决策：references ready 不能只靠 sentinel 非同 TU 命中；默认 `warmup_file=None` 首轮真机仍出现 run1 `2 refs / current_tu`、run2-5 `389 refs / indexed_project`。按 review 要求升级为 query-specific ref-count 稳定判据：先选择非 query、非 probe suffix 的 warmup TU，再对同一个 `find_references` 查询轮询，只有“连续两次引用集签名一致且包含跨 TU 证据”才升级 `indexed_project/complete`；否则仍可返回局部 positive，但 scope 必须是 `current_tu`、health 必须是 `unknown`。
- 2026-07-08 P7 scope 诚实分档更新：此前“未 ready 不返回局部 refs”改为“未证明 cross-TU 时返回局部 refs 但标 `current_tu/unknown`”。原因：review 指出核心问题不是 2 refs 本身，而是 2 refs 被标为 `indexed_project`；B 兜底后即使 ready 判据不充分，也不会谎称项目级覆盖。
- 2026-07-08 P7 review MAJOR 修复后真机验收（复用现有 3593 分片，不重建）：默认 `warmup_file=None`，`CodeGraph.find_references(gst_element_set_state @ gstelement.c:2951, limit=500)` 连续 5 次均稳定 `status=OK`、`index_health=complete`、`total_hits=389`、`381 semantic + 8 not_evidence candidates`、`62 files`、scope 仅 `indexed_project`、`is_exhaustive=False`；耗时分别约 `2.331s / 2.318s / 2.401s / 2.359s / 2.347s`；索引快照前后 `(3593, 39911040, 1781072784911021003)` 不变。
- 2026-07-08 P7 review MAJOR 退化路径真机验收：`background_index=False` 同一查询返回 `status=OK`、`index_health=unknown`、`total_hits=2`、`2 semantic / 0 candidates`、`1 file`、scope 仅 `current_tu`、`is_exhaustive=False`，耗时约 `0.513s`；确认 B 兜底成立，不再把单 TU 的 2 refs 标成 `indexed_project`。
- 2026-07-08 P7 review MAJOR 修复 gate：`tests/test_api.py` -> `33 passed`；全套 `PYTHONPATH=.:tools .venv/bin/python -m pytest tests/ -q` -> `150 passed in 2.76s`；coverage -> `150 passed`，total `93%`，`codegraph/api.py` `94%`，`codegraph/engines/clangd_adapter.py` `100%`，`codegraph/routing.py` `95%`；`.venv/bin/ruff check .`、`.venv/bin/black --check .`、`.venv/bin/mypy codegraph`、`.venv/bin/python -m compileall -q codegraph tools tests`、`git diff --check` 全绿。
