...
- 2026-07-09：四路 review 后 backport P7：P8 真机 plateau 证明 3 次稳定不是 call hierarchy 特例，P7 曾结转的 96/389 plateau 风险也应收紧。将 P7 references 与 P8 call edges 的 ready 判据抽象为共用 `_wait_for_stable_cross_tu_observation()`，统一 `_STABLE_MATCHES=3`；P8 保守度不降，P7 从 2 次稳定升到 3 次稳定。未改 `find_callees` unsupported、CallEdgeResult 方向、relation、scope 兜底。
- 2026-07-09：backport 后 deterministic gate：`tests/test_api.py` -> `42 passed`；全套 `PYTHONPATH=.:tools .venv/bin/python -m pytest tests/ -q` -> `159 passed in 2.44s`；coverage -> `159 passed`，total `93%`，`codegraph/api.py` `95%`，`routing.py` `96%`；ruff/black/mypy/compileall/diff-check 全绿。
- 2026-07-09：backport 后真机硬门：P7 默认 `warmup_file=None` 连续 3 次 `gst_element_set_state` references 均为 `389 refs / 62 files`，耗时 `2.885s / 2.893s / 2.862s`；P8 `gst_element_set_state` callers 仍 `387`；`gst_object_unref` callers 仍 `2951`，limit=1000 页内 `900 semantic + 100 candidates / 254 caller files`；`find_callees(gst_element_set_state)` 仍 `FAILED + CALLHIERARCHY_UNSUPPORTED`；索引快照前后 `(3593, 39911040, 1781072784911021003)` 不变。
