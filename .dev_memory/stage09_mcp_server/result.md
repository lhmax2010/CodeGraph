...
## 最终状态
待用户侧异构 Review 补票。实现、确定性 gate、真实 SDK stdio client 真机验收与 Kimi 独立
Review 已完成，尚未 merge。

## 测试情况
- Baseline：`271 passed in 4.63s`。
- 最终全套：`PYTHONPATH=.:tools .venv/bin/python -m pytest tests/ -q --cov=codegraph --cov-branch`
  -> `324 passed in 6.03s`；总 branch coverage 93%。
- P9 定向：`53 passed`；`codegraph/mcp_server.py` 行覆盖与 branch coverage 均 100%。
- 静态 gate：ruff、black --check、mypy、compileall、git diff --check 全绿。
- 补测内容：四种 status、三类结果、嵌套 credibility/候选/notes/engine_version 全字段 JSON
  roundtrip；未知类型 fail-loud；五工具 schema/映射；错型/越界/allowlist/symlink 输入门禁与 API
  零调用；启动配置；真实 SDK initialize/list/call 与 stdout 协议隔离。
...
- 合入前仍需用户侧补齐异构多路 Review；能力失败不得计为同意票。
