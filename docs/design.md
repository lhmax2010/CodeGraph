...
## 0. 元信息
- 版本：v1.4.1（规范版；前身规范外迭代版至 DESIGN v1.3）
- 创建时间：2026-06-11
...
- **DoD**（引用 §10 验收清单·元数据层）：
  - [ ] credibility.py 扩展 coverage(含 external_known/negative_scope)、active_config(含 mixed)、
        build_config_id、index_health、index_backend、symbol_kind、dependency；新增 INV13–16、INV18–INV21–INV21；
        全部单测通过（含已有 28 不破）
...
- [ ] credibility.py 扩展 coverage(含 external_known/negative_scope)、active_config(含 mixed)、
      ...
      新增 INV13–16、INV18–INV21–INV21（INV17 并入 QR7）；全部单测通过（含已有 28 不破）
...
- [ ] INV20 测试：拒 tree-sitter+not_found / clangd+syntactic+not_found /
      clangd+semantic+blind_spot+not_found 三类；不误杀 clangd_not_found。
      实现顺序：_check_inv20 放 _check_inv12 之后（保 INV12 测试仍先命中 INV12）。
